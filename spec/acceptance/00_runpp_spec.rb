...
describe 'Run manifest with configuration' do
  it 'should apply manifest' do
    ...
    vyatta::protocols::ospf { 'ospf':
      ...
      parameters => { abr-type => 'cisco', router-id => '192.168.21.1' },
      'passive-interface default' => '',
      'passive-interface-exclude eth0' => '',
      'passive-interface-exclude eth1' => '',
      redistribute => {
         bgp => { metric-type => '2', },
         connected => { metric => '1', metric-type => '2' },
         rip => { route-map => 'ROUTE-MAP' },
         static => { metric => '1', metric-type => '2', route-map => 'ROUTE-MAP' }
       }
     }
   }
    EOS
